...
from simulator.core.protofiles.vehicle.body.cabin_climate.v1 import cabin_climate_topics_pb2
...
class CabinClimateService(CovesaService):
  ...
  """
        ...
        for field in field_mask_normalized:
            ...
            if field == "air_distribution_auto_state":
                if request.zone.air_distribution_auto_state not in [
                    ...
                    raise ValidationError(2, f"Zone must be powered off to set air_distribution_auto_state to "
    ...
    """
        ...
        topic = "ultifi:/body.cabin_climate/1/" + zone_name + "#Zone"
        ...
        for field in field_mask:
            ...
            if field == "ac_compressor_setting":
                if (
                        ...
                        cabin_climate_topics_pb2.SystemSettings.CompressorSetting.Value(
                    "CS_UNSPECIFIED")):
            ...
            if self.number_of_zones < 2:
                if field == "sync_all" or field == "sync_rear_to_driver" or field == "rear_zone_lockout":
                    raise ValidationError(2, "sync_all, sync_rear_to_driver, and rear_zone_lockout are not available "
                                             "when there is only 1 zone.")
            if self.number_of_zones < 3:
                if field == "sync_3rdRow_to_driver" or field == "third_row_zone_lockout":
                    raise ValidationError(2, "sync_3rdRow_to_driver and third_row_zone_lockout are not available when "
    ...
    Publishes a system settings message based on the current state
    """
        topic = "ultifi:/body.cabin_climate/1/system_settings#SystemSettings"
...
if __name__ == "__main__":
    ...
    service.start()  # service.wait()  # service.disconnect()
