                    ...
                    enum_fields.append(
                        {
                            ...
                            "enum_values": nested_enum_field["enum_values"],
                        }
                    )
...
def get_res_name(entity_name, config_name):
    for item in autoloader.topic_messages:
        if entity_name in item[0] and config_name + "#" in item[0]:
            ...
            if "." in new_config_name:
                return new_config_name, new_config_name.split(".")[0]
    return config_name, ""
    ...
    for resource_name in resources:
        ...
        if ui_det == {rpc_method: {}}:
            ui_details = [
                {
                    "type": "label",
                    "text": "Type of RPC request is google.protobuf.Empty",
                }
            ]
        else:
